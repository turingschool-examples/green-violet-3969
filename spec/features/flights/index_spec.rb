...
RSpec.describe "Flights Index Page" do
  ...
  let!(:aaflight2) { airline1.flights.create!(number: "1739", date: "08/04/20", departure_city: "Denver", arrival_city: "Ft. Lauderdale")}
  ...
  let!(:aa2_pass5) { FlightPassenger.create!(passenger_id: aapassenger5.id, flight_id: aaflight2.id)}
  let!(:aa2_pass6) { FlightPassenger.create!(passenger_id: aapassenger6.id, flight_id: aaflight2.id)}
  let!(:aa2_pass7) { FlightPassenger.create!(passenger_id: aapassenger7.id, flight_id: aaflight2.id)}
  let!(:aa2_pass8) { FlightPassenger.create!(passenger_id: aapassenger8.id, flight_id: aaflight2.id)}
  ...
  describe "flight's index page" do
    it "displays list of all flight numbers" do
      visit flights_path
      
      within("#flight-#{aaflight1.id}") do
        expect(page).to have_content(aaflight1.number)
        expect(page).to have_content(airline1.name)
        expect(page).to have_content(aa1_pass1.name)
        expect(page).to have_content(aa1_pass2.name)
        expect(page).to have_content(aa1_pass3.name)
        expect(page).to have_content(aa1_pass4.name)
      end
    end

    it "displays name of the airline flight next to each flight number" do
